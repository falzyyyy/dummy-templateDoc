import { useState, useEffect } from 'react';
import api from '../../api/axios';
import { useAlert } from '../../context/AlertContext';

export default function DirectorateList() {
  const [directorates, setDirectorates] = useState([]);
  const [loading, setLoading] = useState(true);
  const [newName, setNewName] = useState('');
  const [editId, setEditId] = useState(null);
  const [editName, setEditName] = useState('');
  const [deleteConfirm, setDeleteConfirm] = useState({ isOpen: false, id: null });
  const { showAlert } = useAlert();

  const fetchDirectorates = () => {
    setLoading(true);
    api.get('/directorates')
      .then(res => setDirectorates(res.data.directorates))
      .catch(err => showAlert('error', 'Gagal memuat direktorat', err.response?.data?.error || err.message))
      .finally(() => setLoading(false));
  };

  useEffect(() => {
    fetchDirectorates();
  }, []);

  const handleAdd = async (e) => {
    e.preventDefault();
    if (!newName.trim()) return;
    try {
      await api.post('/directorates', { name: newName });
      showAlert('success', 'Berhasil', 'Direktorat ditambahkan');
      setNewName('');
      fetchDirectorates();
    } catch (err) {
      showAlert('error', 'Gagal', err.response?.data?.error || 'Terjadi kesalahan');
    }
  };

  const handleUpdate = async (id) => {
    if (!editName.trim()) return;
    try {
      await api.put(`/directorates/${id}`, { name: editName });
      showAlert('success', 'Berhasil', 'Direktorat diperbarui');
      setEditId(null);
      setEditName('');
      fetchDirectorates();
    } catch (err) {
      showAlert('error', 'Gagal', err.response?.data?.error || 'Terjadi kesalahan');
    }
  };

  const handleDelete = (id) => {
    setDeleteConfirm({ isOpen: true, id });
  };

  const executeDelete = async (id) => {
    try {
      await api.delete(`/directorates/${id}`);
      showAlert('success', 'Berhasil', 'Direktorat dihapus');
      fetchDirectorates();
    } catch (err) {
      showAlert('error', 'Gagal', err.response?.data?.error || 'Terjadi kesalahan');
    } finally {
      setDeleteConfirm({ isOpen: false, id: null });
    }
  };

  if (loading && directorates.length === 0) return <div className="p-10 text-center">Memuat...</div>;

  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Kelola Direktorat</h1>
          <p className="text-sm text-slate-500 mt-1">Atur nama divisi atau direktorat perusahaan untuk keperluan filter hak akses.</p>
        </div>
      </div>

      <div className="card p-5 bg-white space-y-6">
        <form onSubmit={handleAdd} className="flex gap-3">
          <input 
            type="text" 
            placeholder="Nama direktorat baru (misal: SDM, Keuangan)..." 
            value={newName} 
            onChange={(e) => setNewName(e.target.value)} 
            className="flex-1 px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#008f51]"
          />
          <button type="submit" className="px-6 py-2 bg-[#008f51] text-white font-semibold rounded-lg hover:bg-[#007a45]">
            Tambah
          </button>
        </form>

        <div className="overflow-x-auto rounded-xl border border-slate-200">
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-700 font-semibold">
              <tr>
                <th className="px-4 py-3">Nama Direktorat</th>
                <th className="px-4 py-3 text-right">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white">
              {directorates.map((d) => (
                <tr key={d.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-800">
                    {editId === d.id ? (
                      <input 
                        type="text" 
                        value={editName} 
                        onChange={(e) => setEditName(e.target.value)} 
                        className="px-2 py-1 border border-[#008f51] rounded w-full focus:ring-1 focus:ring-[#008f51]"
                        autoFocus
                      />
                    ) : (
                      d.name
                    )}
                  </td>
                  <td className="px-4 py-3 flex justify-end gap-2">
                    {editId === d.id ? (
                      <>
                        <button onClick={() => handleUpdate(d.id)} className="text-sm px-3 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200">Simpan</button>
                        <button onClick={() => { setEditId(null); setEditName(''); }} className="text-sm px-3 py-1 bg-slate-100 text-slate-600 rounded hover:bg-slate-200">Batal</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => { setEditId(d.id); setEditName(d.name); }} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                        </button>
                        {d.id !== "1" && (
                          <button onClick={() => handleDelete(d.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                          </button>
                        )}
                      </>
                    )}
                  </td>
                </tr>
              ))}
              {directorates.length === 0 && (
                <tr><td colSpan="2" className="px-4 py-8 text-center text-slate-500">Belum ada direktorat.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      </div>

      {/* Delete Confirmation Modal */}
      {deleteConfirm.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-slide-up">
            <div className="p-6 text-center">
              <div className="w-16 h-16 rounded-full bg-red-100 text-red-500 flex items-center justify-center mx-auto mb-4">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
              </div>
              <h3 className="text-xl font-bold text-slate-800 mb-2">Konfirmasi Hapus</h3>
              <p className="text-sm text-slate-500 leading-relaxed">
                Yakin ingin menghapus direktorat ini? Data yang dihapus tidak dapat dikembalikan.
              </p>
            </div>
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex gap-3 justify-end">
              <button 
                onClick={() => setDeleteConfirm({ isOpen: false, id: null })} 
                className="btn-ghost flex-1 py-2.5 text-sm font-medium"
              >
                Batal
              </button>
              <button 
                onClick={() => executeDelete(deleteConfirm.id)} 
                className="bg-red-600 hover:bg-red-700 text-white rounded-lg px-4 py-2.5 text-sm font-semibold flex-1 shadow-sm transition-colors active:scale-95"
              >
                Ya, Hapus
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
